def search(patterns, multiply=True):
  ...
  for pattern in patterns:
    for i in range(1, len(pattern)):
      ...
      # Check if there is only one difference between first and second
      first_elements = [f_ for f in first for f_ in f]
      second_elements = [s_ for s in second for s_ in s]
      diff = sum(f != s for f, s in zip(first_elements, second_elements))
